...
public class StudioFragment extends Fragment {
    private static final int PICK_VIDEO = 1; // It is a tag to identify out app after leaving it.
    VideoView videoView;
    EditText videoTitle;
    EditText videoDesc;
    Button chooseVideo;
    ...
    private Uri videoUri; //Uniform resource indicator. : it holds the address of video that choosen.
    Context mContext;
    MediaController mediaController; //Use to control the video : play/pause

    //Firebase Instances..
    FirebaseDatabase firebaseDatabase; //realtime database
    DatabaseReference mDatabaseReferences; //use for adding and retrieving data to Realtime database
    StorageReference storageReference; //Use to store the video in database
    StudioData studioData; //Its a model class
    UploadTask uploadTask;  // When the video uploaded on the database, It will allow us to perform some action from the server side.

    //FireStore references..
    FirebaseFirestore db = FirebaseFirestore.getInstance(); // Entry point into the database.
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        storageReference = FirebaseStorage.getInstance().getReference("Video"); //For Firebase Storage
        mDatabaseReferences = firebaseDatabase.getReference("video"); // For realtime Database..
        studioData = new StudioData(); //It is use for storing data into realtime database.

        //When click choose video button, open video chooser activity..
        chooseVideo.setOnClickListener(v -> {
            Intent intent = new Intent(); //use for moving from one page to another
            intent.setType("video/*");
            intent.setAction(Intent.ACTION_GET_CONTENT);
            startActivityForResult(intent, PICK_VIDEO);
        });
        //Handling Publish Button Event..
        publishVideo.setOnClickListener(view1 -> publishVideo());
    }
